...
document.addEventListener("DOMContentLoaded", function () {
    ...
    tblEquipos = $("#tblEquipos").DataTable({
        ajax: {
            url: base_url + "perifericos/listar",
            dataSrc: "",
        },
        columns: [
    ...
    function obtenerNuevoCodigo() {
        ...
        if (!document.querySelector("#id").value) {
            const url = base_url + "perifericos/obtenerNuevoCodigo";
    ...
    frm.addEventListener("submit", function (e) {
        ...
        const url = base_url + "perifericos/registrar";
        const http = new XMLHttpRequest();
...
function eliminarEqu(idEqu) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const url = base_url + "perifericos/delete/" + idEqu;
            const http = new XMLHttpRequest();
            http.open("GET", url, true);
            http.send();
            http.onreadystatechange = function () {
...
function editEqu(idEqu) {
    const url = base_url + "perifericos/edit/" + idEqu;
    const http = new XMLHttpRequest();
    http.open("GET", url, true);
    http.send();
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            document.querySelector("#Usuario_Equipo").value = res.Usuario_Equipo;
...
function agregarImagenes(idEqu) {
    const url = base_url + "perifericos/verGaleria/" + idEqu;
    const http = new XMLHttpRequest();
    http.open("GET", url, true);
    http.send();
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            const res = JSON.parse(this.responseText);
            document.querySelector("#idperifericos").value = idEqu;
            let html = '';
            let destino = base_url + 'assets/images/perifericos/' + idEqu + '/';
...
function eliminar(idEqu, nombre) {
    const url = base_url + "perifericos/eliminarImg";
    const http = new XMLHttpRequest();
    http.open("POST", url, true);
